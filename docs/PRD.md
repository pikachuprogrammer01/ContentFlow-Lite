...
- 密码 bcrypt 哈希存储，永不明文
- 角色权限：super_admin / admin / user
  - super_admin：系统唯一，终端交互创建，可管理全部用户和全部数据，支持批量操作和清空记录
  - admin：注册时提供 ADMIN_SETUP_KEY 获得，可管理普通用户，可访问管理面板查看所有内容/生成记录/模板/版本
  - user：使用内容生成功能，管理自己的 Prompt 模板和 API Key
...
- 自定义 Web 管理面板（/admin），5 个 Tab：用户管理、内容管理、生成记录、Prompt 模板、Prompt 版本
- admin 可查看所有 Tab，但只能管理普通用户
- super_admin 可管理全部用户（修改角色、删除 admin），支持批量删除内容和模板、清空全部生成记录
- 所有数据表格支持分页（可选 10/20/50/100 条/页）和平台中文映射
