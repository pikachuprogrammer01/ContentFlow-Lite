...
const columns: DataTableColumn<AdminUser>[] = [
  ...
  { title: '邮箱', key: 'email' },
  { title: '角色', key: 'role', render: (row) => h('span', { style: roleColor(row.role) }, roleLabel(row.role)) },
  { title: '注册时间', key: 'createdAt', render: (row) => new Date(row.createdAt).toLocaleString() },
  {
    title: '操作', key: 'actions', width: 220,
    render: (row) => h('div', { style: 'display: flex; gap: 8px' }, [
      h(NButton, { size: 'tiny', disabled: !canEdit(row), onClick: () => startEdit(row) }, { default: () => '编辑' }),
      h(NButton, { size: 'tiny', disabled: !canEdit(row), onClick: () => openResetPwd(row) }, { default: () => '重置密码' }),
      canDelete(row)
...
async function saveEdit(): Promise<void> {
  ...
  try {
    const body: Record<string, string> = { username: editUsername.value, email: editEmail.value };
...
async function doResetPwd(): Promise<void> {
  ...
  try {
    await api.post(`/api/admin/users/${resetUserId.value}/reset-password`, { newPassword: newPassword.value });
  ...
  <div>
    ...
    <NModal v-model:show="showEditModal" title="编辑用户" preset="card" style="width: 420px">
      <NSpace vertical>
        ...
        <NSelect v-if="isSuperAdmin" v-model:value="editRole" :options="roleOptions" />
      </NSpace>
    ...
    <NModal v-model:show="showResetPwdModal" title="重置密码" preset="card" style="width: 400px">
      <NSpace vertical>
        <NInput v-model:value="newPassword" type="password" placeholder="输入新密码（至少 6 位）" />
      </NSpace>
      <NSpace justify="end" style="margin-top: 16px">
